...
import net.corda.cdapps.example.flows.TradeFlow;
 ...
 */
@RestController
@RequestMapping("/CordApp") // The paths for HTTP requests are relative to this base path.
public class Controller {
    ...
    @GetMapping(value = "/trades",produces = APPLICATION_JSON_VALUE)
    public List<StateAndRef<TradeState>> getTrades() {
        // Filter by state type: Trade.
        return proxy.vaultQuery(TradeState.class).getStates();
    }

    @PostMapping (value = "create-trade" , produces =  TEXT_PLAIN_VALUE , headers =  "Content-Type=application/x-www-form-urlencoded" )
    public ResponseEntity<String> issueTrade(HttpServletRequest request) throws IllegalArgumentException {
        ...
        try {
            ...
            SignedTransaction result = proxy.startTrackedFlowDynamic(TradeFlow.Initiator.class, amount,otherParty).getReturnValue().get();
            // Return the response.
    ...
    /**
     * Displays all Trade states that only this node has been involved in.
     */
    @GetMapping(value = "my-ious",produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<List<StateAndRef<TradeState>>> getMyIOUs() {
        List<StateAndRef<TradeState>> myious = proxy.vaultQuery(TradeState.class).getStates().stream().filter(
                it -> it.getState().getData().getFromParty().equals(proxy.nodeInfo().getLegalIdentities().get(0))).collect(Collectors.toList());
